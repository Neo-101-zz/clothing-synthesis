...
def cal_all_offset(confs, masks, same):
	...
	for i in range(confs['datasets_num']):
		...
		t_s = time.time()
		x_offset = cal_x_offset(top_mask, bottom_mask, confs['rect_top'], confs['rect_bottom'])
		t_e = time.time()
		print('00' + str(i+1) + ' x calculation completed in ' + str(t_e - t_s))		
		...
		t_e = time.time()
		print('00' + str(i+1) + ' y calculation completed in ' + str(t_e - t_s))		
